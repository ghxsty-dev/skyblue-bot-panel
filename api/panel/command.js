...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { id } = req.query;
    if (id) { const command = await db.getCommand(id); return res.json({ command }); }
  ...
  if (req.method === 'POST') {
    if (req.body.toggle_id) {
      ...
      if (cmd) await db.updateCommand(req.body.toggle_id, { ...cmd, enabled: cmd.enabled ? 0 : 1 });
  ...
  if (req.method === 'DELETE') {
    ...
    if (command) { await db.deleteCommand(id); await db.addLog('command_delete', user.id, user.username, `Komut silindi: /${command.name}`); }
    return res.json({ success: true });
  }
